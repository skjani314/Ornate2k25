...
const Forgotpass = props => {
  ...
  const { success, error, contextHolder } = useContext(EventContext);
  let ismount = true;
  ...
  useEffect(() => {
    ...
    const fun = async () => {
      try {
        ...
        setLoading(true);
        const result = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/user/auth/verify`, { token: token })
        console.log(result);
        setLoading(false);
  ...
  const handleRegSubmit = async (e) => {
    ...
    if (ValData.password && ValData.cpassword) {
      ...
      try {
        ...
        await axios.post(`${import.meta.env.VITE_BACKEND_URL}/user/auth/passchange`, { token: token, data: FormData })
            ...
            (verified) ?
              ...
              !isChanged ?
                ...
                : <div className='fcontainer'>
                  ...
                  <img src="https://i.sstatic.net/1HllQ.png" alt="bheem" className='img-fluid' />
                  <Button><Link to='/home' className="Link">Login Now</Link></Button>
                </div>
          }
        </Col>
      </Row>
